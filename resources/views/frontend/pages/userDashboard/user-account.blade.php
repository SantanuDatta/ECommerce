                                    ...
                                    <ul class="nav flex-column" role="tablist">
                                        ...
                                        <li class="nav-item">
                                            <form method="POST" action="{{ route('logout') }}">
                                                @csrf
                                                <a class="nav-link" href="{{ route('logout') }}" onclick="event.preventDefault(); this.closest('form').submit();"><i class="fi-rs-sign-out mr-10"></i>{{ __('Logout') }}</a>
                                            </form>
                                        </li>
                                    </ul>
                                </div>
                                ...
                                <div class="tab-content account dashboard-content pl-50">
                                    <div class="tab-pane fade active show" id="dashboard" role="tabpanel" aria-labelledby="dashboard-tab">
                                        <div class="card">
                                            <div class="card-header">
                                                <h3 class="mb-0">{{ __('Hello,') }} {{ Auth::user()->name }} {{ Auth::user()->lastName }}</h3>
                                            </div>
                                            <div class="card-body">
                                                <p>
                                                    {{ __('From your account dashboard. you can easily check & view your') }} <a href="#orders">{{ __('recent orders') }}</a>,<br />
                                                    {{ __('manage your') }} <a href="#">{{ __('shipping and billing addresses') }}</a> {{ __('and') }} <a href="#">{{ __('edit your password and account details.') }}</a>
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="tab-pane fade" id="account-detail" role="tabpanel" aria-labelledby="account-detail-tab">
                                        <div class="card">
                                            ...
                                            <div class="card-body">
                                                <form action="{{ route('user.update', Auth::user()->id) }}" method="POST" name="enq">
                                                    @csrf
                                                    <div class="row">
                                                        ...
                                                        <div class="form-group col-md-6">
                                                            ...
                                                            <input class="form-control" name="lastName" type="text" @if (!is_null(Auth::user()->lastName)) value="{{ Auth::user()->lastName }}" @else placeholder="Input Your Last Name" @endif/>
                                                        </div>
                                                        <div class="form-group col-md-6">
                                                        ...
                                                        <div class="form-group col-md-6">
                                                            ...
                                                            <input class="form-control" name="phone" type="phone" @if (!is_null(Auth::user()->phone)) value="{{ Auth::user()->phone }}" @else placeholder="Enter Your Phone No." @endif/>
                                                        </div>
                                                        <div class="form-group col-md-6">
                                                            <label>Address Line 1 <span class="required">*</span></label>
                                                            <input class="form-control" name="addressLineOne" type="text" @if (!is_null(Auth::user()->addressLineOne)) value="{{ Auth::user()->addressLineOne }}" @else placeholder="Input Your First Address" @endif/>
                                                        </div>
                                                        <div class="form-group col-md-6">
                                                            <label>Address Line 2 [Optional]</label>
                                                            <input class="form-control" name="addressLineTwo" type="text" @if (!is_null(Auth::user()->addressLineTwo)) value="{{ Auth::user()->addressLineTwo }}" @else placeholder="Input Your Second Address" @endif/>
                                                        </div>
