                    ...
                    <div class="hero-slider-1 style-4 dot-style-1 dot-style-1-position-1">
                        @foreach ($sliders as $slider)
                            <div class="single-hero-slider single-animation-wrap"
                                style="background-image: url({{ asset('backend/img/sliders/' . $slider->image) }})">
                        ...
                        @foreach ($categories as $pCat)
                            @foreach (App\Models\Category::orderBy('name', 'asc')->where('is_parent', $pCat->id)->get() as $category)
                                @if ($category->is_featured == 1)
                                    <div class="card-2 wow animate__animated animate__fadeInUp @if ($loop->odd) bg-12  @else bg-11 @endif"
                                        data-wow-delay=".1s">
                                        <figure class="img-hover-scale overflow-hidden">
                                            <a href="{{ route('category.product', $category->slug) }}"><img
